...
prefix = "-"
name = "• 𝙔𝙓𝙐 𝘽𝙊𝙏"
rdaftar = "*ANDA SUDAH TERVERIFIKASI*"
rmenu = "𝙔𝙓𝙐 𝘽𝙊𝙏 *_SUDAH TERVERIFIKASI_*"
botinfo = "UNTUK INVITE BOT SILAHKAN DONASI DULU YAA:)"
limitt = 30
memberLimit = 2
ban = []
premium = ["6281977578791@s.whatsapp.net"]
...
async function starts() {
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			const WahyuGans = ["6281977578791@s.whatsapp.net"] // ubah aja gapapa
			...
			mess = {
				wait: '*⏳ Wait sedang di proses dont spam...*',
				success: '*Sukses...*',
				error: {
					stick: ' *Gagal ngab*',
				...
				only: {
					group: '*Maaf command hanya bisa di group!*',
					benned: '*Maaf nomor kamu telah di banned, silahkan hubungi owner untuk selanjutnya*',
					ownerG: '*Maaf command hanya bisa di gunakan oleh owner group!*',
					ownerB: '*Maaf command hanya bisa di gunakan oleh owner Bot!!* ',
					premium: '*Maaf fitur khusus premium!!*',
					userB: `Hai ngab ${pushname2} Kamu Belom Terdaftar Didatabase Silahkan Ketik \n${prefix}daftar`,
					admin: '*Maaf command hanya bisa di gunakan oleh admin group!!*',
					Badmin: '*Maaf command hanya bisa digunakan ketika BOT jadi admin!*',
					publikG: `*Maaf ngab bot skrng privat*\n*Untuk lebih jelasnya ketik*\n*${prefix}infobot*`
				}
			}

			const botNumber = frhan.user.jid
			const ownerNumber = ["6281977578791@s.whatsapp.net"] // owner number ubah aja
       ...
       if (messagesLink.includes("://chat.whatsapp.com/")){
		...
		case 'verify':
					frhan.updatePresence(from, Presence.composing)
					if (isUser) return reply('kamu sudah daftar di YXU BOT:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
            ...
            break  
				...
				case 'owner':
                 ...
                 frhan.sendMessage(from, 'Chat Aja ngab Owner nya gans:v',text, { quoted: mek} )
                 break
                case 'fitnah':
                 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           frhan.sendMessage(from, options, text, { quoted: mek } )
				...
				if (!isPublic) return reply(mess.only.publikG)
					watak = body.slice(1)
					wa =["penyayang","pemurah","Pemarah","Pemaaf","Penurut","Baik","baperan","Baik Hati","penyabar","Uwu","top deh, pokoknya","Suka Membantu","SANGEAN"]
				...
				if (!isPublic) return reply(mess.only.publikG)
					hobby = body.slice(1)
					hob =["Memasak","Membantu Atok","Mabar","Nobar","Sosmed an","Membantu Orang lain","Nonton Anime","Nonton Drakor","Naik Motor","Nyanyi","Menari","Bertumbuk","Menggambar","Foto fotoan Ga jelas","Maen Game","Coli","Colmek","Berbicara Sendiri"]
         ...
         case 'lirik':
                ...
                if (!isPublic) return reply(mess.only.publikG)
                    ...
                    frhan.sendMessage('6281977578791@s.whatsapp.net', options, text, {quoted: mek})
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 YXU BOT 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
			    if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 YXU BOT 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat YXU BOT\`\`\``)
					break
                                case 'bcgc':
					frhan.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							frhan.sendMessage(_.jid, bc, image, {caption: `[ *YXU BROADCAST* ]\n\n${body.slice(4)}`})
						}
