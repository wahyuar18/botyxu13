...
prefix = "!"
name = "• YXU BOT"
rdaftar = "*ANDA SUDAH TERVERIFIKASI DI DATABASE*"
		...
		case 'verify':
            ...
            break  
				...
				case 'owner':
                 ...
                 frhan.sendMessage(from, 'Langsung chat aja ngab kalo perlu sve:v',text, { quoted: mek} )
                 break
                case 'fitnah':
                 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
		    ...
		    case 'map':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *YXU BOTBROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
