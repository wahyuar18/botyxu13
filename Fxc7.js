...
prefix = "!"
name = "• YXU BOT"
rdaftar = "*ANDA SUDAH TERVERIFIKASI DI PERANGKAT YXU BOT"
rmenu = "*_YXU BOT SUDAH TERVERIFIKASI_*"
	...
	frhan.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hi ngab @${num.split('@')[0]} 👋🏻\nWelcome to group *${mdata.subject}*\n––––––––––––––––––––––––\nKetik *${prefix}verify* untuk verifikasi nomor anda ke database\nKetik *${prefix}menu* untuk melihat fitur bot`
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				success: '*Sukses...*',
				error: {
					stick: ' *Failed, terjadi kesalahan saat mengkonversi sticker*',
				...
				only: {
					...
					userB: `Hi ngab *${pushname2}* Kamu Belom Terdaftar Didatabase Silahkan Ketik \n${prefix}verify`,
       ...
       if (messagesLink.includes("://chat.whatsapp.com/")){
		...
		setTimeout( () => {
		reply(`*「 LINK GROUP DETECTED 」*\nLink Group Terdeteksi maaf *${pushname2}* anda akan di kick`)
		}, 0)
		}
		...
		case 'verify':
            ...
            break  
				...
				case 'owner':
                 ...
                 frhan.sendMessage(from, 'Jika mau sve ya sve aja ngab, owner aing cogan:v',text, { quoted: mek} )
                 break
                case 'fitnah':
                 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           frhan.sendMessage(from, options, text, { quoted: mek } )
                ...
                if (!isUser) return reply(mess.only.userB)
					...
					const tag = {
					text: `@${nom.split("@s.whatsapp.net")[0]} *_tag!_*`,
		    ...
		    case 'map':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *YXU BOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						frhan.sendMessage(from, buffer, image, {quoted: mek, caption: '_sukses mengkonversi sticker ke image_'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender) 
					break 
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Tidak Terdaftar Di Dalam Database *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[YXUBOT]','aqua'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
